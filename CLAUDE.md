...
- **Two browser projects.** `chromium` runs the behaviour specs at desktop size and ignores `mobile.spec.ts`; `mobile-chromium` runs only that file, on a Pixel 5 viewport. The mobile spec is the guard on Phase 4.1: every personal route is checked for content past the edge of the screen, and the task row's edit and delete controls must be visible without a hover, since a touch screen never hovers. The overflow check walks the DOM element by element rather than reading `documentElement.scrollWidth` — `body` carries `overflow-x: clip`, which is what stops a stray wide child panning the page and would also hide it from a `scrollWidth` check.
...
- **Shared UI logic lives in `lib/`, not copied between components**: [lib/task-status.ts](lib/task-status.ts) (the status cycle and its icons), [lib/use-task-status.ts](lib/use-task-status.ts) (optimistic toggle with rollback, used by all five components that render a task), [lib/category-colour.ts](lib/category-colour.ts) (a subcategory inherits its top-level parent's colour, plus the default swatch). [components/layout/AppShell.tsx](components/layout/AppShell.tsx) is the chrome both route groups share — the layouts under `app/(dashboard)` and `app/(household)` are ten lines each and differ only in which nav they pass. Each of these was previously duplicated five or six times and had begun to drift; add to them rather than re-deriving the logic in a component.
...
- [x] 4.1 Mobile-optimised layouts — designed for phone viewport throughout
...
- [x] Playwright smoke suite (PR #8) and the pre-4.1 consolidation refactor (PR #9) — both were listed at the end of [SECURITY_HARDENING.md](SECURITY_HARDENING.md)
- [x] Phase 4.1 complete (25 Aug 2026) — mobile layouts throughout. The header renders the section nav inline on desktop and as a swipeable strip under a sticky bar on a phone ([components/nav/SectionNav.tsx](components/nav/SectionNav.tsx)); every control that was revealed by `group-hover` now shows below `md`, because on a touch screen those edit and delete buttons did not exist at all; task rows stack category and horizon under the title where the columns are hidden; the calendar and planner sidebars stack instead of taking 224px beside the grid, and drag activation moved to [lib/dnd-sensors.ts](lib/dnd-sensors.ts) so a swipe starting on a task chip scrolls rather than dragging. Guarded by the `mobile-chromium` Playwright project
- [ ] **Next: 4.2 Progressive Web App** — manifest, service worker, installable on Android. Two small manual security items still open in [SECURITY_HARDENING.md](SECURITY_HARDENING.md): enable leaked password protection in Auth → Policies (dev + prod), and identify `public.rls_auto_enable()`
